...
O projeto está pronto para deploy na **Vercel** via GitHub.

### Passo 1 — Subir o código para o GitHub

```bash
# Dentro da pasta sgep/
git init
git add .
git commit -m "SGEP v3 — versão inicial"
git branch -M main
git remote add origin https://github.com/<seu-usuario>/sgep.git
git push -u origin main
```

> Se preferir, crie o repositório vazio primeiro em [github.com/new](https://github.com/new) e copie a URL gerada para o `git remote add origin`.

### Passo 2 — Importar na Vercel

1. Acesse [vercel.com/new](https://vercel.com/new) e faça login com a conta do GitHub.
2. Clique em **Import** no repositório `sgep`.
3. A Vercel detecta o Next.js automaticamente. As configurações já vêm corretas (também fixadas em `vercel.json`):
   - **Framework Preset:** Next.js
   - **Build Command:** `next build`
   - **Install Command:** `npm install`
   - **Output Directory:** `.next`
4. Nenhuma variável de ambiente é necessária nesta versão (estado em memória).
5. Clique em **Deploy**.

Ao final, a Vercel publica o sistema em uma URL `https://sgep-<hash>.vercel.app`. Cada novo `git push` na branch `main` dispara um redeploy automático.
